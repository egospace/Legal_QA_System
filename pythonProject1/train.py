...
# Get data features
# input_ids, input_masks, segment_ids=[2000, 60]  label_ids=[2000, 15]
# 2000表示训练数据大小, 60表示序列长度, 15表示分类个数
...
with sess.as_default():
    ...
    for i in range(hp.num_train_epochs):  # num_train_epochs=50
        ...
        for j in range(num_batchs-1):
            ...
            # Log
            if j % hp.print_step == 0:  # print_step=100
                fd = {MODEL.input_ids: input_id_,
                      MODEL.input_masks: input_mask_,
                      MODEL.segment_ids:segment_id_,
                      MODEL.label_ids:label_id_}
